...
public class Player : MonoBehaviour, IDamageable, ICanInvul, IHasAttributes
{
    ...
    int _maxMana = 100;
    int _mana;
    int _level = 35; // TODO TESTING
    int _experience = 0;
    int _expRequiredForCurrentLevel = 0;
    ...
    public const int _statsPerLevel = 5;
    ...
    public delegate void InvulnerableCallback();
    ...
    void SetupPlayer()
    {
        ...
        CalculateStats();
        _expRequiredForCurrentLevel = GetExperienceRequired();
    ...
    public void LevelUp()
    {
        ...
        print("You leveled up to level " + _level);
        // play animation, make a sound, whatever
        // bing bang boom levelup WOOOO
    }

    public void AddExperience(int expIn)
    {
        if (_level == LookupDictionary.instance.maxLevel)
            return;

        _experience += expIn;

        while(_experience >= _expRequiredForCurrentLevel)
        {
            _experience -= _expRequiredForCurrentLevel;
            LevelUp();
            _expRequiredForCurrentLevel = GetExperienceRequired();
