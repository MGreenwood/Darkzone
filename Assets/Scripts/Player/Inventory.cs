...
public class Inventory : MonoBehaviour
{
    ...
    private float screenWidth, screenHeight; // used to track screen resizing and adjusting inventory
    ...
    private void Update()
    {
        ...
        {
            AddItem(Instantiate(Resources.Load<Item>("ScriptableObjects/Items/Weapon/ShortSwords/spellsword") as Weapon));
        }
    ...
    {        
        ...
        Vector2 offset = new(position.x - corners[1].x, corners[1].y - position.y);

        Index ndx = new((int)(offset.x / globalSizeOfTile),(int)(offset.y / globalSizeOfTile));
    ...
    {
        // set parent
        Image img = Instantiate(itemImagePrefab).GetComponent<Image>();
        ...
        // set size of image
        int sWid = item.GetItemSize()._width;
        int sHig = item.GetItemSize()._height;
        img.rectTransform.localScale = Vector3.one;

        float wid = localSizeOfTile * sWid, hig = localSizeOfTile * sHig;
        img.GetComponent<InventoryItemObject>().SetSize(wid, hig);
        /*
        RectTransform imgTransform = imgObject.GetComponent<InventoryItemObject>().GetRarityOutline().GetComponent<RectTransform>();
        imgTransform.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, localSizeOfTile * wid);
        imgTransform.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, localSizeOfTile * hig);
        */
        ...
        // set sprite of image
        img.GetComponent<InventoryItemObject>().GetImage().sprite = item.GetSprite();
    }
